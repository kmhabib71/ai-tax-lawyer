  ...
  fixTableHeaders(text) {
    ...
    return (
      text
        ...
        .replace(
          /শিরনামা\s*সংখ্যা.*?শুল্কহার/gs,
          "\n=== TAX TABLE ===\nHeading | HS Code | Description | Rate (%)\n"
        )

        // Fix column indicators
        .replace(/\(১\)\s*\(২\)\s*\(৩\)\s*\(৮\)/g, "(1) | (2) | (3) | (4)")
...
async function fixAllTaxTables() {
  ...
  if (results.length > 0) {
    ...
    results.forEach((result) => {
      console.log(
        `   - ${path.basename(result.outputFile)} (${result.chunks} chunks)`
      );
    });
